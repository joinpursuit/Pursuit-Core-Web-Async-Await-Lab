...
async function main() {
  ...
  async function drawCards() {
    ...
    for (const card of cards) {
      ...
      cardImage.addEventListener("click", () => clickCard(cardImage));
  ...
  async function clickCard(cardImage) {
    const drawCardResponse = await axios(
      `https://deckofcardsapi.com/api/deck/${deckId}/draw/?count=1`
    );
    const { cards, remaining } = drawCardResponse.data;
